...
async function seedPerformanceCheck() {
    ...
    const existingId = new Set(existingRecords.map((record) =>record.performance_id));

    let insertedCount = 0;
    for (const performanceInput of SEED_PERFORMANCE_CHECK) {
        if (existingId.has(performanceInput.performance_id)) {
            continue;
        }

        await createPerformance(performanceInput);
        insertedCount += 1;
    }
...
seedPerformanceCheck().catch((error) => {
